...
import { View, Text, TouchableOpacity, StyleSheet, Image, FlatList, Platform, ScrollView } from 'react-native';
            ...
            <ScrollView>
                ...
                <View style={{ marginTop: hp(2), marginHorizontal: wp(2) }}>
                    ...
                    <View style={styles.testViewWrapper}>
                        <Text style={styles.headingText}>My Tests</Text>
                        <View>
                    ...
                    <FlatList
                        ...
                        horizontal={true}
                        renderItem={({ item }) =>
                            <TouchableOpacity key={item.id} style={[styles.testMobileWrapper, { backgroundColor: '#F8F8F8' }]}>
                                ...
                                <View style={styles.lockWrapper}>
                                    <Image source={item.imageLock} style={{ height: 12, width: 12,marginLeft:Platform.OS == 'web' ? wp(.5):wp(0) }} resizeMode='contain' />
                                    <Text style={styles.imageHeadingText}>{item.imageHeading}</Text>
                                </View>
                            </TouchableOpacity>}
                    />
                    <View style={styles.bloodWrapper}>
                        <View style={{ flexDirection: 'row' }}>
                            <Image source={require('../src/assets/icons/blood-test.png')} style={{ height: Platform.OS == 'web'? hp(6):  hp(7), width: Platform.OS == 'web'? wp(4): wp(20), resizeMode: 'contain', }} />
                            <View style={{ flexDirection: 'column' , marginLeft:wp(1)}}>
                                <Text style={styles.bloodHeading}>Blood Report</Text>
                                <Text style={styles.bloodSubHeading}>Add your report so that we can diagnose you better</Text>
                            ...
                            <Image source={require('../src/assets/icons/add_circle.png')} style={{ height: hp(4), width:  Platform.OS == 'web'? wp(2):  wp(8), resizeMode: 'contain', }} />
                        </View>
                        </View>
...
const styles = StyleSheet.create({
    ...
    testMobileWrapper: {
        width: Platform.OS == 'web'? wp(10): wp(39.5),
    ...
    imageHeadingText: {
        ...
        fontSize: 10,
        color: '#868686',
        paddingLeft:Platform.OS == 'web' ? wp(.5):  wp(1.5)
    },
    bloodWrapper: {
        borderWidth: 1,
        borderColor: '#D9D9D9',
